...
Verify connectivity using `psql` from either a bastion host or within the AKS network, ensuring your credentials are valid. Replace the placeholders with your actual values:
```bash
...
An Azure AD application must be granted read/write access to the Blob Storage container. CloudPrem will use this identity to interact with storage.
